#!/usr/bin/env python3
"""
任务 A2.3 补充: 0HP→2HP迁移任务
创建时间: 2026-08-07
目标: 重新运行之前失败的0HP→2HP迁移任务
原因: 原脚本因缺少source_pretrain_0hp.pt文件而失败
解决方案: 创建符号链接 source_pretrain_0hp.pt -> source_pretrain.pt
方法:
    1. 0HP→2HP迁移（源域0HP，目标域2HP）
    2. 运行SHOT、TENT、RPSWD三种方法
    3. 在Clean、0dB、-3dB三个SNR水平下测试
    4. 每种配置10个种子
"""
...
def main():
    print("=" * 80, flush=True)
    print(f"任务 A2.3 补充: 0HP→2HP迁移任务", flush=True)
    ...
    for snr in snr_levels:
        ...
        for method_name, method_func in methods.items():
            ...
            print(f"      平均: Acc={results[snr_key][method_name]['mean_accuracy']:.2f}±{results[snr_key][method_name]['std_accuracy']:.2f}%, "
                  f"IR={results[snr_key][method_name]['mean_ir_recall']:.2f}±{results[snr_key][method_name]['std_ir_recall']:.2f}%", flush=True)

    # 保存结果
    output_data = {
        'task': 'A2.3_supplement',
        'description': '0HP to 2HP Migration Task (Supplement)',
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'migration': f'{source_hp}_to_{target_hp}',
    ...
    output_path = RESULTS_DIR / 'task_A2_3_0HP_to_2HP_supplement.json'
    with open(output_path, 'w') as f:
        json.dump(output_data, f, indent=2)

    print(f"\n{'=' * 80}", flush=True)
    print(f"✓ 结果已保存: {output_path}", flush=True)
    print(f"✓ 任务 A2.3 补充完成: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    print(f"{'=' * 80}", flush=True)
