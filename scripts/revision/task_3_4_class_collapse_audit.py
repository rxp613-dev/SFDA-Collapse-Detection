...
Task 3-4: 类崩溃审计
Created: 2026-08-06
Purpose: 审计不同方法在不同SNR下的类级别性能退化
Config:
  - 方法: SHOT, TENT, NRC, SAR, RPSWD
  - SNR: Clean, 0dB, -3dB, -6dB
  - 分析每个类别的recall变化
  - 总运行次数: 5方法 × 4SNR × 10seeds = 200次
...
def main():
    ...
    results = {
        'task': '3-4',
        'description': 'Class collapse audit',
    ...
    for snr in snr_levels:
        ...
        if snr == 'Clean':
            ...
            noisy_samples = add_gaussian_noise(samples, -6)

        for method_name, method_func in methods.items():
            results['results'][snr][method_name] = {}
            ...
            for seed in seeds:
                ...
                accuracy, metrics = method_func(
                    bb, clf, noisy_samples, labels,
                    seed=seed
                )

                print(f"Acc={accuracy:.2f}%, Normal={metrics['Normal']['recall']:.2f}%, IR={metrics['IR']['recall']:.2f}%, Ball={metrics['Ball']['recall']:.2f}%, OR={metrics['OR']['recall']:.2f}%")

                results['results'][snr][method_name][f'seed_{seed}'] = {
                    'accuracy': accuracy,
                    'Normal': metrics['Normal']['recall'],
                    'IR': metrics['IR']['recall'],
                    'Ball': metrics['Ball']['recall'],
                    'OR': metrics['OR']['recall']
                }
    ...
    for snr in snr_levels:
        ...
        for method_name in methods.keys():
            ...
            for seed in seeds:
                ...
                seed_result = results['results'][snr][method_name][seed_key]
                accuracies.append(seed_result['accuracy'])
                normal_recalls.append(seed_result['Normal'])
                ir_recalls.append(seed_result['IR'])
                ball_recalls.append(seed_result['Ball'])
                or_recalls.append(seed_result['OR'])

            statistics[snr][method_name] = {
                'accuracy_mean': np.mean(accuracies),
                'accuracy_std': np.std(accuracies),
                'Normal_mean': np.mean(normal_recalls),
                'Normal_std': np.std(normal_recalls),
                'IR_mean': np.mean(ir_recalls),
                'IR_std': np.std(ir_recalls),
                'Ball_mean': np.mean(ball_recalls),
                'Ball_std': np.std(ball_recalls),
                'OR_mean': np.mean(or_recalls),
                'OR_std': np.std(or_recalls)
            }

            print(f"  {snr} {method_name}: "
                  f"Acc={statistics[snr][method_name]['accuracy_mean']:.2f}±{statistics[snr][method_name]['accuracy_std']:.2f}%, "
                  f"Normal={statistics[snr][method_name]['Normal_mean']:.2f}±{statistics[snr][method_name]['Normal_std']:.2f}%, "
                  f"IR={statistics[snr][method_name]['IR_mean']:.2f}±{statistics[snr][method_name]['IR_std']:.2f}%, "
                  f"Ball={statistics[snr][method_name]['Ball_mean']:.2f}±{statistics[snr][method_name]['Ball_std']:.2f}%, "
                  f"OR={statistics[snr][method_name]['OR_mean']:.2f}±{statistics[snr][method_name]['OR_std']:.2f}%")

    results['statistics'] = statistics

    # 保存结果
    output_path = RESULTS_DIR / 'task_3_4_class_collapse_audit.json'
    with open(output_path, 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n{'=' * 80}")
    print(f"✓ Results saved to: {output_path}")
    print(f"✓ Task 3-4 completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'=' * 80}")
