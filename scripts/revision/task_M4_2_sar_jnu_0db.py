#!/usr/bin/env python3
"""
任务: M4.2 - 在JNU数据上运行SAR方法（0dB噪声，10个种子）
日期: 2026-08-10
目标: 验证SAR在JNU数据集上的表现，补充论文的第二数据集实验
方法:
  1. 加载JNU源模型和目标数据
  2. 添加0dB AWGN噪声
  3. 运行SAR方法（10个种子：42-51）
...
def run_sar(backbone, classifier, samples, labels, num_epochs=50, lr=1e-3, seed=42):
    """
    SAR方法实现
    基于论文: Gong et al., "Towards Stable Test-Time Adaptation in Dynamic Wild World"
    SAR使用sharpness-aware minimization进行熵最小化
    """
...
def main():
    print("=" * 80)
    print("任务 M4.2: 在JNU数据上运行SAR方法（0dB噪声）")
    print("=" * 80)
    ...
    # 运行SAR方法（10个种子）
    print(f"\n4. 运行SAR方法（10个种子：42-51）")
    results = {}
    seeds = list(range(42, 52))

    for i, seed in enumerate(seeds):
        print(f"\n   种子 {seed} ({i+1}/10):")
        metrics = run_sar(backbone, classifier, noisy_samples, labels,
    ...
    output_path = RESULTS_DIR / 'task_M4_2_sar_jnu_0db.json'
