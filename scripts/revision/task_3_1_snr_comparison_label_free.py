#!/usr/bin/env python3
"""
Task 3-1: SNR-Dependent Performance Comparison (Label-Free)
Created: 2026-07-22
Purpose: 重做Table 8实验，所有方法使用pseudo-labels
...
def main():
    print("=" * 80)
    print("Task 3-1: SNR-Dependent Performance Comparison (Label-Free)")
    print("=" * 80)
    ...
    results = {
        'task': '3-1',
        'description': 'SNR-Dependent Performance Comparison (Label-Free)',
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'note': 'All methods use pseudo-labels (SFDA compliant)',
        'snr_levels': {}
    }
    ...
    for snr in snr_levels:
        ...
        for method_name, method_func in methods.items():
            ...
            for seed in seeds:
                acc, ir, confusion_matrix, macro_f1, balanced_acc, per_class_metrics = method_func(bb, clf, noisy_samples, labels, seed=seed)
                ...
                print(f"  Seed {seed}: Acc={acc:.2f}%, IR={ir:.2f}%, Macro-F1={macro_f1:.2f}%, BalAcc={balanced_acc:.2f}%")
    ...
    out_file = RESULTS_DIR / 'task_3_1_snr_comparison_label_free_v2.json'
    with open(out_file, 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n{'=' * 80}")
    print(f"✓ Results saved to: {out_file}")
    print(f"✓ Task 3-1 completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'=' * 80}")
