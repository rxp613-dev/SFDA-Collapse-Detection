#!/usr/bin/env python3
"""
任务 Minor.6: 扩展RPSWD OR双峰实验到50个种子
创建时间: 2026-08-11
目标: 将RPSWD OR recall双峰实验从10个种子扩展到50个种子，验证双峰比例的稳定性
方法:
    1. 在Clean数据上运行RPSWD适应（50个种子，seed 42-91）
    2. 记录每个种子的OR recall
    3. 分析双峰分布的稳定性
    4. 与原始10个种子的结果对比
"""
...
def run_rpswd(backbone, classifier, samples, labels, num_epochs=100, lr=1e-4, seed=42):
    ...
    for epoch in range(num_epochs):
        for batch_x, _ in loader:
            ...
            # 计算 boundary scores
            similarities = torch.mm(features_norm, prototypes.t())
            boundary_scores, _ = similarities.max(dim=1)

            # 计算 soft weights
            soft_weights = torch.sigmoid(-boundary_scores * 10.0)

            # 计算 loss
            ce_loss = F.cross_entropy(logits_temp, pseudo_labels, reduction='none')
            weighted_ce = (ce_loss * soft_weights).mean()

            # Repulsion loss
            repulsion_loss = 0.0
            for c1 in range(NUM_CLASSES):
                for c2 in range(c1 + 1, NUM_CLASSES):
                    if prototypes[c1].norm() > 0 and prototypes[c2].norm() > 0:
                        dist = F.cosine_similarity(prototypes[c1].unsqueeze(0), 
                                                  prototypes[c2].unsqueeze(0))
                        repulsion_loss += (1 - dist).squeeze()

            loss = weighted_ce + 0.5 * repulsion_loss / (NUM_CLASSES * (NUM_CLASSES - 1) / 2)
...
def main():
    print("=" * 80)
    print("任务 Minor.6: 扩展RPSWD OR双峰实验到50个种子")
    print("=" * 80)
    ...
    output_data = {
        'task': 'Minor.6',
        'description': 'Expand RPSWD OR bimodal experiment to 50 seeds',
        'num_seeds': 50,
        'seed_range': [42, 91],
    ...
    output_path = OUTPUT_DIR / 'task_Minor_6_rpswd_or_bimodal_50seeds.json'
