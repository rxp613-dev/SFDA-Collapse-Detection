...
方法:
    ...
    2. 使用与CWRU相同的模型架构
    3. 划分训练集和验证集 (80%/20%)
    4. 训练模型 100 epochs（减少epochs以加快训练）
    5. 保存最佳模型检查点
"""
...
BATCH_SIZE = 256  # 增大batch size以加快训练
NUM_EPOCHS = 100  # 减少epochs
LEARNING_RATE = 1e-3
NUM_CLASSES = 4
...
def main():
    print("=" * 80, flush=True)
    print(f"任务 A1.4: 在PU上训练源域模型（重新训练）", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"  训练集: {len(train_samples)} 样本", flush=True)
    print(f"  验证集: {len(val_samples)} 样本", flush=True)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 打印进度
        if (epoch + 1) % 10 == 0:
            print(f"  Epoch {epoch+1}/{NUM_EPOCHS}: Train Loss={train_loss:.4f}, Train Acc={train_acc:.2f}%, Val Acc={val_acc:.2f}%, Best Val Acc={best_val_acc:.2f}%", flush=True)

    print(f"\n训练完成!", flush=True)
    print(f"  最佳验证准确率: {best_val_acc:.2f}%", flush=True)
    print(f"  模型已保存: {checkpoint_path}", flush=True)
    ...
    print(f"\n✅ 训练报告已保存: {report_path}", flush=True)

    print("\n" + "=" * 80, flush=True)
    print("✅ 任务 A1.4 完成", flush=True)
    print("=" * 80, flush=True)
