#!/usr/bin/env python3
"""
Comprehensive Corrected SNR Sweep — All Methods × All SNR Levels
Created: 2026-08-14
Purpose: Re-run the full SNR comparison experiment using Step 3 corrected implementations
         to replace the OLD (pre-correction) data in table10, and provide data for
         table2 (cliff), table3 (migration), table8 (denoising), table9 (adaptive lr).
Input: Source model pretrained on CWRU 0HP
Output: JSON file with accuracy, macro_f1, balanced_acc, IR_recall for each
        method × SNR level × seed configuration
Dataset: CWRU (0HP→3HP)
SNR Levels: Clean, +6dB, +3dB, 0dB, -3dB, -6dB
         + SHOT cliff: +2dB, +1dB, -1dB, -2dB (fine-grained around 0dB)
         + SHOT optimal lr sweep: also at lr=1e-4
Methods: SHOT (corrected), TENT, NRC (corrected), SAR (corrected), RPSWD
Seeds: 42-44 (3 seeds per configuration, matching Step 3)
GPU: Yes (CUDA enabled)
Epochs: 30 (matching Step 3)

Key corrections from Step 3:
...
def main():
    """Main function: comprehensive corrected SNR sweep"""
    print("=" * 80, flush=True)
    print("Comprehensive Corrected SNR Sweep — All Methods × All SNR Levels")
    print("=" * 80, flush=True)
    ...
    # before any method runs. This ensures:
    # 1. Same noise for 0dB as in compute_table1_metrics.py (for Table 1 consistency)
    #    — 0dB MUST be generated first, matching the order in compute_table1_metrics.py
    # 2. Reproducible noise for all SNR levels
    # 3. No random-state drift between method calls
    NOISE_SEED = 2026
    ...
    # Generate 0dB FIRST to match compute_table1_metrics.py noise realization
    pregenerated_noise = {}
    pregenerated_noise[0] = add_gaussian_noise(cwru_samples, 0)
    print(f"  Pre-generated 0dB noise (first, for Table 1 consistency)", flush=True)

    # Then generate remaining SNR levels
    remaining_snr_levels = [float('inf'), 6, 3, -3, -6, 2, 1, -1, -2]
    for snr_db in remaining_snr_levels:
        pregenerated_noise[snr_db] = add_gaussian_noise(cwru_samples, snr_db)
    print(f"  Pre-generated noise for {len(remaining_snr_levels)} additional SNR levels", flush=True)

    methods = ["SHOT", "TENT", "NRC", "SAR", "RPSWD"]

    # Define SNR level groupings (using pre-generated noise)
    standard_snr_levels = [float('inf'), 6, 3, 0, -3, -6]
    standard_snr_names = ["Clean", "+6dB", "+3dB", "0dB", "-3dB", "-6dB"]
    cliff_snr_levels = [2, 1, 0, -1, -2]
    cliff_snr_names = ["+2dB", "+1dB", "0dB", "-1dB", "-2dB"]
    shot_optimal_lr = 1e-4

    # ===========================================
    # Part 1: Standard SNR sweep (all methods)
    # ===========================================
    print("\n[2/5] Standard SNR sweep (all methods, default lr)...", flush=True)
    standard_results = {}

    total_runs = len(methods) * len(standard_snr_levels) * len(seeds)
    run_idx = 0

    for snr_db, snr_name in zip(standard_snr_levels, standard_snr_names):
        print(f"\n  SNR: {snr_name} ({snr_db}dB)", flush=True)
        samples_noisy = pregenerated_noise[snr_db]  # Use pre-generated noise
    ...
    # ===========================================
    # Part 2: SHOT at optimal lr (standard SNR levels)
    # ===========================================
    print("\n[3/5] SHOT at optimal lr=1e-4 (standard SNR levels)...", flush=True)
    shot_optimal_results = {}
    ...
    for snr_db, snr_name in zip(standard_snr_levels, standard_snr_names):
        ...
        for seed in seeds:
            acc, mf1, bacc, ir = run_shot_corrected(
                ...
                samples_noisy, cwru_labels,
                num_epochs=num_epochs, lr=shot_optimal_lr, seed=seed
            )
            accuracies.append(acc)
            macro_f1s.append(mf1)
            balanced_accs.append(bacc)
            ir_recalls.append(ir)
            print(f"    SHOT lr={shot_optimal_lr:.0e} seed={seed} SNR={snr_name}: "
                  f"Acc={acc:.2f}%, Macro-F1={mf1:.2f}%, BalAcc={bacc:.2f}%, IR={ir:.2f}%", flush=True)

        shot_optimal_results[snr_name] = aggregate_results(
            accuracies, macro_f1s, balanced_accs, ir_recalls
        )

    # ===========================================
    # Part 3: SHOT cliff localization (fine-grained SNR)
    # ===========================================
    print("\n[4/5] SHOT cliff localization (fine-grained SNR, lr=1e-3)...", flush=True)
    cliff_results = {}

    for snr_db, snr_name in zip(cliff_snr_levels, cliff_snr_names):
        samples_noisy = pregenerated_noise[snr_db]  # Use pre-generated noise
        accuracies, macro_f1s, balanced_accs, ir_recalls = [], [], [], []
        ...
        for seed in seeds:
            ...
            balanced_accs.append(bacc)
            ir_recalls.append(ir)
            print(f"    SHOT cliff lr=1e-3 seed={seed} SNR={snr_name}: "
                  f"Acc={acc:.2f}%, IR={ir:.2f}%", flush=True)

        cliff_results[snr_name] = aggregate_results(
            accuracies, macro_f1s, balanced_accs, ir_recalls
        )

    # ===========================================
    # Part 4: Source model baseline at all SNR levels
    # ===========================================
    print("\n[5/5] Source model baseline at all SNR levels...", flush=True)
    source_backbone.eval()
    source_classifier.eval()
    baseline_results = {}

    for snr_db, snr_name in zip(standard_snr_levels, standard_snr_names):
        samples_noisy = add_gaussian_noise(cwru_samples, snr_db)
        with torch.no_grad():
            preds = source_classifier(source_backbone(samples_noisy.to(device)))[1].argmax(dim=1)
            metrics, accuracy, cm, macro_f1, balanced_acc, ir_recall = compute_metrics(
                preds, cwru_labels
            )
        baseline_results[snr_name] = {
            "accuracy": float(accuracy),
            "macro_f1": float(macro_f1),
            "balanced_acc": float(balanced_acc),
            "ir_recall": float(ir_recall),
        }
        print(f"    Source model SNR={snr_name}: Acc={accuracy:.2f}%", flush=True)

    # ===========================================
    # Compile and save results
    # ===========================================
    output = {
        "task": "Comprehensive Corrected SNR Sweep",
        ...
        "config": {
            ...
            "shot_optimal_lr": float(shot_optimal_lr),
        },
        "source_model_baseline": baseline_results,
        "standard_snr_sweep": standard_results,
        "shot_optimal_lr_sweep": shot_optimal_results,
        "shot_cliff_localization": cliff_results,
    }

    output_path = RESULTS_DIR / "comprehensive_corrected_snr_sweep.json"
    ...
    # Print summary
    print(f"\n{'=' * 80}", flush=True)
    print("Summary: Standard SNR Sweep (default lr)", flush=True)
    ...
    print("-" * 68, flush=True)
    for snr_name in standard_snr_names:
        row = f"{snr_name:>8s}"
        for m in methods:
    ...
    print(f"\n{'=' * 80}", flush=True)
    print("Summary: SHOT Optimal LR Sweep (lr=1e-4)", flush=True)
    print(f"{'=' * 80}", flush=True)
    for snr_name in standard_snr_names:
        acc = shot_optimal_results[snr_name]["accuracy_mean"]
        print(f"  {snr_name:>8s}: {acc:.2f}%", flush=True)

    print(f"\n{'=' * 80}", flush=True)
    print("Summary: SHOT Cliff Localization (lr=1e-3)", flush=True)
    print(f"{'=' * 80}", flush=True)
    for snr_name in cliff_snr_names:
        acc = cliff_results[snr_name]["accuracy_mean"]
        ir = cliff_results[snr_name]["ir_recall_mean"]
        print(f"  {snr_name:>8s}: Acc={acc:.2f}%, IR={ir:.2f}%", flush=True)

    print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
