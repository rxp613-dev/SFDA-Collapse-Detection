#!/usr/bin/env python3
"""
Task B1.4: Re-run JNU Main Audit with Confusion Matrix Saving
Created: 2026-08-08 13:30
Purpose: 重跑JNU主审计（A1.5），保存混淆矩阵以计算正确的macro-F1和balanced accuracy
Methods: SHOT, TENT, RPSWD
SNR Levels: Clean, 0dB, -3dB
Seeds: 42-51 (10 seeds)
Total runs: 3 methods × 3 SNR × 10 seeds = 90 runs
...
def main():
    print("=" * 80, flush=True)
    print("Task B1.4: Re-run JNU Main Audit with Confusion Matrix Saving", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"Label distribution: {torch.bincount(labels).tolist()}", flush=True)

    methods = {
        'SHOT': lambda s, l, seed: run_shot(backbone, classifier, s, l, seed=seed),
        'TENT': lambda s, l, seed: run_tent(backbone, classifier, s, l, seed=seed),
        'RPSWD': lambda s, l, seed: run_rpswd(backbone, classifier, s, l, seed=seed),
    }
    ...
    results = {
        'metadata': {
            'task': 'B1.4_rerun_jnu_main_audit_with_confusion',
    ...
    # Save results
    output_file = RESULTS_DIR / 'task_B1_4_rerun_jnu_main_audit_with_confusion.json'
